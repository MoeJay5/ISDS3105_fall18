...
options(warn = -1)
tbl(con, 'hotel') %>%
  distinct(hotelType) -> q1 #query is saved 
collect(q1) #becomes local table now 
```
...
tbl(con, sql('select authorNickname, h.hotelId, hotelName from hotel h join review r join author a where h.hotelId = r.hotelId and r.authorid = a.authorid order by authorNickname'))
...
select(hotelId, hotelName, reviewLocation)
