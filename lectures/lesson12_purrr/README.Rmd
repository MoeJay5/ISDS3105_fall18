...
map(.x = iphoneSales, .f = ~ mean(.x, na.rm = T))
...
t %>% mutate(nums = purrr::map(.x = nums, 2, .default = as.character(NA)))
...
bad_driversMountain <-
 bad_drivers %>%
         mutate(isMountain = case_when(
         state %in% mountainStates ~ 'Mountain',
         state %in% c('Louisiana', 'Mississippi', 'Alabama', 'Georgia', 'South Carolina') ~ 'South',
         T ~ 'Other'),
         state = tolower(state))

bad_driversMountain %>%
split(.$isMountain) %>%
 purrr::map(.x = ., ~ right_join(.x, states, by = c('state' = 'region')) %>%
                     filter(!is.na(insurance_premiums)) %>%
                     ggplot() +
                     geom_polygon(aes(x = long, y = lat, group = group, fill = insurance_premiums), color = "white") + coord_fixed(1.3) + labs(title = paste('Insurance premiums')))
```
