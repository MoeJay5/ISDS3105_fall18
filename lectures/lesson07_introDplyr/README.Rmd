...
```{r}
sn_revenue %>%   select(company, y2015)
```
...
drug_use %>%
  ...
  gather(key = 'drug', value = 'usage', alcohol_use, cocaine_use, marijuana_use) %>%
    ggplot(data = .) +
...
fivethirtyeight::drug_use %>%
 select( age, alcohol_use, cocaine_use, marijuana_use,n) %>%
 gather(key = 'DrugType', value = 'percent',-age,-n) %>%
 filter(age %in% c('18','19','20','21','22-23')) %>%
 ggplot() + geom_bar(aes(x = DrugType, y = percent, fill = DrugType), stat = 'identity') + facet_wrap(~age) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
