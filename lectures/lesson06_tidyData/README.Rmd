...
```{r}
sn_users %>%
  gather(key = "socialNet", value = 'Mil.Users', -year) %>%
ggplot(aes(x = year, y =  Mil.Users, color = socialNet)) +
  geom_line()
```
...
#untidyReview %>%
#  gather(key = "Attributes", value = 'Rating', -reviewId, -reviewStayYear)

# -reviewId and -reviewStayYear excludes them from being gathered to one column

gather(untidyReview, key = 'Attributes', value = 'Rating', -reviewId, -reviewStayYear) %>%
ggplot() +
...
gather(iphoneSales, key = 'Quarter', value = 'Sales', -year) %>%
 ggplot() +
 geom_line(aes(x= Sales, y = year)) +
 facet_wrap(~Quarter)
```
